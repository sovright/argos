...
| T-S6 | Wallet-file passphrase (zcashd `wallet.dat` / ZecWallet Lite import) leaks via memory, disk, logs, or CLI argv | H | ⚠️ | Held as `SecretString` end to end, never written to disk, never accepted as a CLI flag (which would leak to shell history and `ps` — prompt-only via `dialoguer::Password` in `crates/zeck-cli/src/main.rs`), and never logged. Decrypted key material zeroizes on drop. `secrecy` zeroizes on drop but does not `mlock`; the passphrase remains reachable from swap and core dumps, as for the seed. See `docs/secret-memory-evaluation.md`. **Wallet-file import is CLI-only today** — the GUI exposes no wallet-file entry point, so there is currently no passphrase crossing the Tauri IPC boundary. When one is added it will be a **new instance of accepted audit Issue A** (plaintext JSON over IPC, already accepted for the seed phrase); recorded here in advance so that crossing is a deliberate decision rather than something inherited silently. |
